...
```bash
px pull                              # list visible ontologies
...
output predicates, datasource refs resolve, context bodies exist, etc. Also **warns** when an
ontology ships an ontology schema but has no concepts (a concept-less ontology renders empty on the
...
### `px delete ONTOLOGY`
Permanently delete a whole ontology (a server ontology id or exact name) and everything in it —
...
> concrete ontology is always required (there is no "delete everything" mode).
...
| Option | Meaning |
|---|---|
| `-p, --ontology TEXT` | Only plan the named ontology(s), by name / slug / id. Repeatable. |
...
Creates the ontology if new; snapshots each ontology first (best-effort).

| Option | Meaning |
|---|---|
| `-p, --ontology TEXT` | Only apply the named ontology(s). Repeatable. |
...
- **Best-effort concepts:** a concept whose body references something that can't be resolved
  ...
  ontology, the ontology schema, and apps still apply, and the run exits non-zero with a skip summary.
  A genuine error (parse, conflict) still aborts fast.
- Cross-account port: a recreated ontology's new id is written back to `prometheux.yaml`, and
  app `ontology.id` references + `disk/results/<id>` sibling-output paths are retargeted at the
  new ontology automatically.
...
| Option | Meaning |
|---|---|
| `-p, --ontology TEXT` | Limit the lookup to the named ontology(s). Repeatable. |
...
```
my-workspace/
  prometheux.workspace.yaml     # lists the shared context vault + ontologies
  ...
  ontologies/
    credit-risk/
      ...
      context/                  # this ontology's project-scoped context
...
context: ./context
ontologies:
  - ./ontologies/credit-risk
  - ./ontologies/fraud-detection
```
...
**Duplicate an ontology (same account or another):**
```bash
px pull <id> --out /tmp/copy
# edit /tmp/copy/ontologies/<slug>/prometheux.yaml: remove `id:`, change the name
px apply /tmp/copy -y        # creates a new ontology; app/results paths retargeted automatically
```

**Port a file-based ontology across accounts (files travel):**
```bash
...
- **Ontology id** is written into `ontologies/<slug>/prometheux.yaml` on create — commit it so
  re-apply targets the same ontology.
...
- **Keep the `id` in `prometheux.yaml`** — it targets the same ontology on re-apply. If it's lost,
  `apply` adopts an existing same-name ontology (reconcile-by-name) instead of duplicating, so a
  unique ontology name makes recovery reliable.
