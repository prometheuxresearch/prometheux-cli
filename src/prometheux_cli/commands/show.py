...
def show(concept, path, project_selectors, page, page_size, order_by, as_json):
    ...
    project, _local = _resolve_concept(projects, concept)
    if project is None:
    ...
    try:
        resp = px.fetch_results(
            project.id, concept, page=page, page_size=page_size,
            scope=project.scope, order_by=order_by,
        )
