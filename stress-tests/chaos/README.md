...
| `02-kill-mid-apply.sh` | Interrupted `apply` + retry → exactly one ontology (no duplicate) | Yes |
...
# Account harnesses (they create + delete their own throwaway ontologies):
...
Each account harness tears down every ontology it created (by id). If a run is itself killed,
